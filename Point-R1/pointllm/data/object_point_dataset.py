...
class ObjectPointCloudDataset(Dataset):  # 定义点云数据集类，继承自PyTorch的Dataset基类
    ...
    def __getitem__(self, index):  # 定义获取数据项的方法，实现Dataset接口
        ...
        if self.point_indicator in sources[0]['conversations'][0]['value']:  # 如果第一个对话中包含点云指示符
            ...
            if self.tokenizer is None:  # 如果分词器为None
                data_dict = dict(  # 创建数据字典
                    point_clouds=torch.from_numpy(point_cloud.astype(np.float32)),  # 将点云转换为PyTorch张量
                    object_ids=object_id  # 保存对象ID
                )
        ...
        data_dict = preprocess_v2(  # 使用v1版本预处理函数
            sources,  # 传入数据源
